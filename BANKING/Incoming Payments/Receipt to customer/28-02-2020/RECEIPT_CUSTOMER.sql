...
create VIEW [dbo].[RECEIPT_CUSTOMER]
AS
select RCT2.InvType,
...
Case when ORCT.DocCurr='INR' THEN rct2.SumApplied ELSE RCT2.AppliedFC END as InvPaidAmt,
...
ocrn.CurrCode,ocrn.F100Name 'HUNDRETH NAME'

,OCRN.frgnname 'hundredth'
,ocrn.CurrName 'CURRENCY NAME', orct.Comments									
...
left join 									
(									
select DocNum,DocEntry,DocDate,CardCode,CardName,Series,ObjType,
Case when DocCur='INR' THEN DocTotal else DocTotalFC end as DocTotal,
Case when DocCur='INR' THEN DocTotal-paidtodate else DocTotalFC-paidfc end as paidamount
,WTSum,VatSum,TotalExpns,NumAtCard from oinv 
union all									
select DocNum,DocEntry,DocDate,CardCode,CardName,Series,ObjType
,Case WHEN DocCur='INR' THEN (-1)*DocTotal ELSE (-1)*DocTotalFC end as DocTotal,
Case when DocCur='INR' THEN DocTotal-paidtodate else DocTotalFC-paidfc end as paidamount,WTSum,VatSum,									
TotalExpns,NumAtCard from orin									
union all									
select Number DocNum,TransId DocEntry,RefDate,NULL CardCode,NULL CardName,Series,TransType,null DocTotal,0 paidamount,									
